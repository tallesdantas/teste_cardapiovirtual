...
// salvar modelos de formatos json para importa��o
...
////////////////////////////////pega a data do tablet///////////////////////
function rtdt(callback){
		
		navigator.globalization.dateToString(
				  new Date(),
				  function (date) {callback(date.value);},
...
/////////////////// função remover acentos palavras /////////////////
...
//////////////////////////////////retorna o select box para  valor selecionado ao selecionar ///////
...
////////////////// checa se existe conex�o no aparelho //////////////
...
function checar_internet(){
		...
		if((states[networkState] == 'No network connection') || (states[networkState] == 'Unknown connection')){
			//navigator.notification.alert(" N�o foi detectado conex�o com a internet ", null, "Conex�o", "Prosseguir");
...
function downloadFile(showdialog,filepath,callback){
			
		if(showdialog == 1){
			$('#header_progress').html('Progresso:');
			$('#header_progress').show();
	        ...
	        var localFileName = remoteFile.substring(remoteFile.lastIndexOf('/')+1);
			 window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, function(fileSystem) {
		            fileSystem.root.getFile(localStorage['sys_pastaimagens']+'/'+localFileName, {create: true, exclusive: false}, function(fileEntry) {
...
/////////////////////fun��o que des zipa e/ou  exclui o zip //////////////////////////


function unzipexclude(nome,exclui)
{

	var ZipClient = new ExtractZipFilePlugin();
	ZipClient.extractFile("sdcard/"+localStorage['sys_pastaimagens']+"/"+nome, function(){alert('descompactados');}, function(){alert('erro unzip');} );

	if(exclui == 0){
		
	    window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, function(fileSystem) {
			fileSystem.root.getFile(localStorage['sys_pastaimagens']+"/"+nome, {create: false, exclusive: false}, function(fileEntry){
			    console.log(fileEntry);
			    fileEntry.remove(function(){ alert('zip removido'); }, function(){ alert('erro');} );
			}, function(){ alert('erro');} );
		    }, function(){ alert('erro');} );
	}
...
////////////////// fun��o que pega o text to speech e poe numa imput text ////////////
function textofaladoit(idtextfield) {
    ...
    window.plugins.speechrecognizer.startRecognize(function(result){
		...
		$.each(resultado, function(i, item) {
			//alert(idtextfield);
			//alert(item);
			string += '<li style="margin: 15px; border-radius: 1em;" onclick="editarit(\''+idtextfield+'\',\''+item+'\',null)" >'+item+'</li>';
		});
		//editarit('+idtextfield+','+item+',null); $(document).trigger(\'simpledialog\', {\'method\':\'close\'});
		$(document).simpledialog2({  mode: 'blank',
			headerText: 'Selecione uma op��o',
			headerClose: true,
			buttonPrompt: 'Selecione uma op��o',
			
			 blankContent : 
			  "<ul data-role='listview'>"+string+"</ul>"+
...
//////////////////////////// FUN��ES COM ELEMENTOS HTML ///////////////////////////<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<___________________
